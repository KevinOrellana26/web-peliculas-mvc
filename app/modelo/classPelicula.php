...
class Pelicula extends Modelo
{
    ...
    { //devuelve todos los comentarios de la pelicula
        $consulta = "SELECT comentarios.id_comentario, comentarios.contenido, usuarios.nombre_usuario, comentarios.fecha FROM peliculas INNER JOIN comentarios ON peliculas.id_pelicula=comentarios.id_pelicula INNER JOIN usuarios ON comentarios.id_usuario=usuarios.id_usuario WHERE comentarios.id_pelicula = :id_pelicula";
        $result = $this->conexion->prepare($consulta);
